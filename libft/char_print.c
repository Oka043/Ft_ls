...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/31 13:26:58 by olaktion          #+#    #+#             */
/*   Updated: 2018/10/31 13:26:59 by olaktion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			print_str(t_arg *arg)
{
	...
	if (F(F_L) || arg->conv == 'S')
	{
		...
		if (len < 0 || !(out = (char *)ft_memalloc(len + 1)))
			return (-1);
		(F(F_UTF)) ? ft_tout_str(out, ws, len) : ft_wctouc(out, ws, len);
